...
; Checks to see if the first string begins with the second string
...
(define (string-starts-with? String1 String2)
  (cond[(and (>= (string-length String1) (string-length String2))
             (string=? (substring String1 0 (string-length String2)) String2)) #true]
       [(and (>= (string-length String1) (string-length String2))
             (string=? String2 "")) #true]
       [(and (>= (string-length String1) (string-length String2))
             (string=? String1 String2)) #true]
       [else #false]))
...
(define (door-state-template DoorState)
  (...
   (cond [(string=? DoorState DS-CLOSED) ...]
         [(string=? DoorState DS-LOCKED) ...]
         [(string=? DoorState DS-OPEN) ...]) ...))
...
(define (draw-door DoorState)
  (cond [(equal? DoorState DS-CLOSED) DOOR-CLOSED]
        [(equal? DoorState DS-OPEN) DOOR-OPEN]
        [(equal? DoorState DS-LOCKED) DOOR-LOCKED]))
...
(check-expect (change-locked-door "q") DS-LOCKED)
...
(check-expect (change-open-door "f") DS-OPEN)
